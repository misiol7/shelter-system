...
def daily_minutes(db,dog_id):
    ...
    walks=db.query(Walk).filter(Walk.dog_id==dog_id, Walk.start_time>=start, Walk.start_time<end).all()
    total=0
    for w in walks:
...
def dogs(db:Session=Depends(get_db)):
    ...
    for d in db.query(Dog).all():
        out.append({"id":d.id,"name":d.name,"available":d.available,"status":d.status,"daily_minutes":daily_minutes(db,d.id)})
    return out

@app.post("/walk/start")
async def start(dog_id:int, db:Session=Depends(get_db)):
    ...
    dog.status="WALKING"
    db.add(Walk(dog_id=dog_id, admin_id=1))
    db.commit()
    await ws.broadcast({"type":"DOG_UPDATE"})
    return {"ok":True}

@app.post("/walk/stop")
async def stop(dog_id:int, db:Session=Depends(get_db)):
    dog=db.query(Dog).get(dog_id)
    walk=db.query(Walk).filter(Walk.dog_id==dog_id, Walk.end_time==None).first()
    if not dog or not walk: raise HTTPException(404,"No walk")
...
def add_dog(name:str, admin=Depends(current_admin), db:Session=Depends(get_db)):
    d=Dog(name=name); db.add(d); db.commit(); return {"ok":True}

@app.post("/admin/dogs/{dog_id}/availability")
def avail(dog_id:int, available:bool, admin=Depends(current_admin), db:Session=Depends(get_db)):
    d=db.query(Dog).get(dog_id)
    if not d: raise HTTPException(404,"Not found")
    d.available=available
    if not available: d.status="UNAVAILABLE"
    elif d.status=="UNAVAILABLE": d.status="IN_KENNEL"
    db.commit()
    return {"ok":True}
